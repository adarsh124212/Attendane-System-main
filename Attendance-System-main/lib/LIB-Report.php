...
class Report extends Core {
  ...
  function attend($code) {
    ...
    $course = $this->DB->fetch(
      "SELECT *, DATE_FORMAT(`course_start`, '.DT_LONG.') `sd`, DATE_FORMAT(`course_end`, '.DT_LONG.') `ed`
       FROM `courses`
       WHERE `course_code`=?",
      [$code]
    );
    ...
    if (!$course) {
      // Course not found, handle error or return
      return;
    }
    ...
    while ($r = $this->DB->stmt->fetch()) {
      $class[$r["class_id"]] = $r["cd"];
      $classdate[] = $r["cd"];
    }
    fputcsv($f, $classdate);

    // (A4) ASSUME "ABSENT" IF NO ATTENDANCE RECORDS ARE FOUND
    $preattend = array_fill_keys(array_keys($class), 0);
